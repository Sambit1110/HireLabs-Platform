    ...
    if (type === 'embedding') {
      return (
        <div className="hl-feature-visual hl-embedding-visual">
          ...
          <div className="hl-vector-grid" aria-hidden="true">
            {Array.from({ length: 72 }).map((_, index) => {
              const scale =
                0.55 + ((index * 17) % 46) / 100;

              const opacity =
                0.35 + ((index * 13) % 60) / 100;
  ...
  return (
    ...
    >
      <style>{`
         ...
         * IMPORTANT:
         *
         * The vector area has four fixed rows.
         * Each cell has a fixed layout height.
         * Visual variation comes from scaleY(), not
         * from changing the actual height.
         *
         * This prevents the cells from spilling into
         * the Semantic / Searchable / Private row.
         */
